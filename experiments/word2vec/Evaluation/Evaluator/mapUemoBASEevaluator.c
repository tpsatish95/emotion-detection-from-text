...
char *TestEmo[6] = {"anger","disgust","fear","joy","sadness","surprise"};
...
while(!feof(emoF))
{
    ...
    a = 0;
    while (!feof(emoF)) {
      st1[a] = tolower(fgetc(emoF));
      if ((st1[a] == ' ') || (a >= max_size - 1)) {
        st1[a] = 0;
        break;
      }
      if ((st1[a] == '\n')) {
...
if(LineEND==1)
{
  // indicates a valuation line over

if(strcmp(prevE,TestEmo[po])==0)
       success[po]++;
else 
  fail[po]++;
total[po]++;
...
accu[po]=(float)success[po]/(float)total[po];


}
int p1=0;
for(p1=0;p1<6;p1++)
printf("Sucess rate: %d\nTotal Cases: %d\nThe Accuracy for %s is : %g\n",success[p1],total[p1],TestEmo[p1],accu[p1]*100);
printf("Sucess rate: %f\n",success[p1]/(success[p1]+fail[p1]));  
printf("Sucess rate: %f\n",sqrt(accu[p1]*success[p1]/(success[p1]+fail[p1])));
  return 0;
}
