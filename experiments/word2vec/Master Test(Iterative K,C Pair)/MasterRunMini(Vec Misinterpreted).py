...
for ki in k:
    print("Bk===" +str(ki))
    t0 = time.time()
    kmeans = cluster.MiniBatchKMeans(n_clusters=ki,n_init=100,batch_size = ki*5).fit(X_Scaled_Feature_Vecs)
    print(str(time.time()-t0))
    print(kmeans.inertia_)
    print("Donek===" +str(ki))
    ##### save best for future use
    save_obj(kmeans,"MbestKMEANSforK"+str(ki))

    KM = load_obj("MbestKMEANSforK"+str(ki))
    ...
    for feeli in feel:  
        ...
        for sp in range(5):
            hisvec = []
            DumpP = open(feeli+"Mh2v"+str(sp)+".pkl","wb")
            w = open("SP/"+feeli+str(sp)+".txt","r")
            print(feeli+str(sp))
            lines = w.readlines()
            w.close()
            for line in lines:
               ...
               ## print(words)
                ...
                for token in tokens:
                    try:
                        Svec.append(KM.predict(model[token])[0])
                    except:
                        ###Svec.append(-1)
            ...
            print(feeli+ str(sp) +" and MSkip for Test also Dumped!!")
            DumpP.close()
        i=i+1
    ...
    for ci in C:   #for each C
        for inte in range(5):           ## for each of 5 cases
            ...
            for feeli in feel:
                for feee in range(5):
                    if feee !=skip[inte]:
                        pkl= open(feeli+"Mh2v"+str(feee)+".pkl","rb")
                        while 1:
                            try:
                                ...
                                break
            svc_ = svm.SVC(kernel = "rbf", C=ci,cache_size = 2048)
            ### chi2 kernel needs memory to be computed so use rbf for now
            ### Memory error with chi2kernel checkout
            print("SVM Training "+ str(svc_.kernel)+"SVMmodel"+str(inte)+"c"+str(ci)+"kM"+str(ki))
            #print(train_X)
            svc_.fit(train_X,train_y)  

            save_obj(svc_,"SVMmodel"+str(inte)+"c"+str(ci)+"kM"+str(ki))
    ...
    for ci in C:
        avgALL5 = []
        for inte in range(5):           ## for each of 5 cases
            SVMM = load_obj("SVMmodel"+str(inte)+"c"+str(ci)+"kM"+str(ki))
            count = [0,0,0,0,0,0]
            scount = [0,0,0,0,0,0]
            fi = 0
            for feeli in feel:
                skk = open(feeli+"Mh2v"+str(skip[inte])+".pkl","rb")           
                while 1:
                        try:
        ...
        for eemo in range(6):
            ...
            for ra in range(5):
                t=t+avgALL5[ra][eemo]
            avgg.add(t/5)
        store = open("RM"+str(ki)+"C"+str(ci)+".txt","w")
        for eemo in range(6):
